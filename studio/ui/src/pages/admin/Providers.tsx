...
import { AlertTriangle, BrainCircuit, CheckCircle2, KeyRound, Loader2, Pencil, Plus, Plug, Star, Trash2, XCircle } from "lucide-react";
    ...
    <Page>
      ...
      {q.isLoading ? (
        ...
        <div className="space-y-4">
          {q.data.map((p) => (
            <ProviderCard key={p.id} p={p} isAdmin={isAdmin} onEdit={() => setEditing(p)} />
          ))}
        </div>
      )}
      {editing && <ProviderDialog provider={editing === "new" ? null : editing} onClose={() => setEditing(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["providers"] })} />}
    </Page>
  );
}

function ProviderCard({ p, isAdmin, onEdit }: { p: Provider; isAdmin: boolean; onEdit: () => void }) {
  const qc = useQueryClient();
...
function ProviderDialog({ provider, onClose, onSaved }: { provider: Provider | null; onClose: () => void; onSaved: () => void }) {
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return provider ? api.put(`/providers/${provider.id}`, body) : api.post("/providers", body);
    },
    onSuccess: () => {
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="ID do modelo no provedor">
              <Input value={f.modelId} onChange={(e) => set("modelId", e.target.value)} required className="font-mono text-xs" placeholder="deepseek-chat" />
            </Field>
